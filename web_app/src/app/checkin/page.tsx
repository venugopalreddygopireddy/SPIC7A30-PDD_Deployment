...
export default function CheckInScreen() {
  ...
  useEffect(() => {
    try {
      ...
      const parsedSleep = parseFloat(sleep.toFixed(1)) || 0;
      
      let autoQuality = 1;
      if (parsedSleep >= 8) autoQuality = 5;
      else if (parsedSleep >= 6.5) autoQuality = 4;
      else if (parsedSleep >= 5) autoQuality = 3;
      else if (parsedSleep >= 4) autoQuality = 2;
      
      setFormData(prev => ({ 
        ...prev, 
        sleep_duration: parsedSleep,
        sleep_quality: autoQuality
      }));
    } catch(e) {}
  ...
  if (isAnalyzing) {
    return (
      <div className="min-h-screen bg-[#4A3B69] flex flex-col items-center justify-center p-6 font-sans">
        ...
        <h1 className="text-3xl font-bold text-white mb-3 text-center">{t('Analyzing Your Data')}</h1>
  ...
  return (
    <div className="min-h-screen bg-[#11131C] text-slate-200 font-sans flex flex-col relative pb-24">
      <div className="px-6 py-5 flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-white flex-1">{t('Daily Wellness Check-In')}</h1>
      </div>
      ...
      <div className="flex-1 px-4 md:px-6 overflow-y-auto">
        <div className="max-w-2xl mx-auto bg-[#1C1E2B] rounded-3xl p-6 shadow-xl mb-6">
          <div className="flex items-center gap-4 mb-4">
            {stepInfo.icon}
            <h2 className="text-2xl font-bold text-white">{t(stepInfo.title)}</h2>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 space-y-4">
                ...
                <div className="space-y-2">
                  <p className="font-semibold text-white">Sleep Quality (1-5): {formData.sleep_quality}</p>
                  <div className="flex gap-4">
                    {[1,2,3,4,5].map(q => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-colors ${q <= formData.sleep_quality ? 'bg-[#82e0aa] text-[#111116]' : 'bg-[#212330] border border-slate-700 text-slate-300 hover:border-[#82e0aa] hover:text-[#82e0aa]'}`}
                      >
                        {q}
                      </button>
                    ))}
          ...
          {currentStep === 3 && (
            <div className="space-y-8">
              ...
              <div>
                ...
                <CustomSlider min={0} max={960} step={1} value={Math.round(formData.screen_time * 60)} onChange={(v:any) => updateField('screen_time', v / 60)} />
              </div>
              <div>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-[#222533] border-t border-slate-800 p-4 md:p-6 flex items-center justify-between z-10">
        <span className="text-white font-medium pl-2">{t('Step')} {currentStep} / 6</span>
        <button
          onClick={handleNext}
        ...
        >
          {loading && !isAnalyzing ? <Loader2 className="animate-spin" size={20} /> : (
            <>
              {currentStep === 6 ? t('Submit Analysis') : t('Next')}
