...
export default function Dashboard() {
  ...
  const [latestCheckinDetails, setLatestCheckinDetails] = useState<any>(null);
  const [dashboardSummary, setDashboardSummary] = useState<DashboardSummaryResponse | null>(null);

  const [showExportModal, setShowExportModal] = useState(false);
  const [exportTimePeriod, setExportTimePeriod] = useState('Last 30 Days');
  ...
  const generateNotifications = () => {
    ...
    notifs.push({
      id: 'welcome',
      title: 'Welcome to CortiSense! 🌟',
      message: 'Ready to master your stress? Let’s start tracking!',
      time: new Date(now.getTime() - 48 * 60 * 60 * 1000).toISOString(),
      type: 'engagement'
    });

    if (history && history.length > 0) {
      history.slice(0, 10).forEach((checkin: any) => {
        const date = new Date(String(checkin.timestamp).endsWith('Z') ? checkin.timestamp : checkin.timestamp + 'Z');
        
        if (checkin.score >= 60) {
          notifs.push({
            id: `stress-${checkin.id}`,
            title: checkin.score >= 80 ? 'High Stress Detected 🚨' : 'Elevated Stress ⚠️',
            message: 'We noticed your stress levels are high. Take a moment to breathe.',
            time: new Date(date.getTime() + 5 * 60000).toISOString(),
            type: 'stress'
          });
        }
        
        notifs.push({
          id: `reminder-${checkin.id}`,
          title: 'Time for a Check-in! 🕒',
          message: 'How are you feeling right now? Log your mood.',
          time: new Date(date.getTime() - 2 * 60 * 60 * 1000).toISOString(),
          type: 'checkin'
        });
      });
    } else {
      notifs.push({
        ...
        message: 'Don\'t forget to log your stress levels today!',
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        setLoading(true);
        const [hist, weekly, monthly, trends, factors, dashboard] = await Promise.all([
          getHistory().catch(() => []),
        ...
        if (hist && hist.length > 0) {
          getCheckinDetails(hist[0].id).then(details => setLatestCheckinDetails(details)).catch(() => {});
        }

        let calculatedWeekly = weekly;
        ...
        if (hist && hist.length > 0) {
          ...
          const recentWeek = hist.filter((c: any) => {
            const dateStr = String(c.timestamp).endsWith('Z') ? String(c.timestamp) : String(c.timestamp) + 'Z';
            return new Date(dateStr) >= startOfWeek;
          });

          const recentMonth = hist.filter((c: any) => {
            const dateStr = String(c.timestamp).endsWith('Z') ? String(c.timestamp) : String(c.timestamp) + 'Z';
            return new Date(dateStr) >= startOfMonth;
          });

          const scoresW = recentWeek.map((c: any) => c.score);
          let lowW = 0, modW = 0, highW = 0;
          recentWeek.forEach((c: any) => {
            if (c.stress_level.toLowerCase().includes('low')) lowW++;
            else if (c.stress_level.toLowerCase().includes('moderate')) modW++;
            else highW++;
          });

          calculatedWeekly = {
            avg_score: scoresW.length ? Math.floor(scoresW.reduce((a: number,b: number)=>a+b,0)/scoresW.length) : 0,
            highest_score: scoresW.length ? Math.max(...scoresW) : 0,
            lowest_score: scoresW.length ? Math.min(...scoresW) : 0,
            total_checkins: recentWeek.length,
            distribution: { low: lowW, moderate: modW, high: highW }
          };

          const scoresM = recentMonth.map((c: any) => c.score);
          let lowM = 0, modM = 0, highM = 0;
          const calActivity: Record<string, number> = {};
          recentMonth.forEach((c: any) => {
            if (c.stress_level.toLowerCase().includes('low')) lowM++;
            else if (c.stress_level.toLowerCase().includes('moderate')) modM++;
            else highM++;
            const dateStr = String(c.timestamp).endsWith('Z') ? String(c.timestamp) : String(c.timestamp) + 'Z';
            const d = new Date(dateStr);
            const dStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
            if (!calActivity[dStr] || calActivity[dStr] < c.score) {
              calActivity[dStr] = c.score;
            }
          });

          calculatedMonthly = {
            avg_score: scoresM.length ? Math.floor(scoresM.reduce((a: number,b: number)=>a+b,0)/scoresM.length) : 0,
            lowest_score: scoresM.length ? Math.min(...scoresM) : 0,
            highest_score: scoresM.length ? Math.max(...scoresM) : 0,
            total_checkins: recentMonth.length,
            distribution: { low: lowM, moderate: modM, high: highM },
            calendar_activity: calActivity
          } as any;
        }

        setHistory(hist);
        if (calculatedWeekly) setWeeklyData(calculatedWeekly);
        if (calculatedMonthly) setMonthlyData(calculatedMonthly);
  ...
  const stressScore = latestCheckIn?.score ?? dashboardSummary?.latest_stress_score ?? 0;
  const stressCategory = latestCheckIn?.stress_level ?? dashboardSummary?.latest_stress_category ?? 'High';
  const todayStr = new Date().toDateString();
  const todayCheckinsCount = history.filter((c: any) => {
    const d = new Date(String(c.timestamp).endsWith('Z') ? String(c.timestamp) : String(c.timestamp) + 'Z');
  ...
  let currentStreak = 0;
  if (history.length > 0) {
    const uniqueDates = Array.from(new Set(history.map((c: any) => {
      const d = new Date(String(c.timestamp).endsWith('Z') ? String(c.timestamp) : String(c.timestamp) + 'Z');
  ...
  const latestSleepDuration = latestCheckinDetails?.sleep_duration ?? dashboardSummary?.latest_sleep_duration ?? 0;
  ...
  const greeting = new Date().getHours() < 12 ? t('Good Morning') : new Date().getHours() < 17 ? t('Good Afternoon') : t('Good Evening');
  ...
  const renderMonthly = () => {
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 flex flex-col items-start">
            ...
            <span className="text-white font-bold text-xl">
              {(monthlyData as any).lowest_score ?? (Object.values(monthlyData.calendar_activity).length > 0 ? Math.min(...Object.values(monthlyData.calendar_activity)) : 0)}
            </span>
            <span className="text-slate-400 text-[10px] uppercase font-bold mt-1">Lowest Score</span>
          </div>
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 flex flex-col items-start">
            <div className="text-rose-500 mb-2"><TrendingUp size={16} /></div>
            <span className="text-white font-bold text-xl">
              {(monthlyData as any).highest_score ?? (Object.values(monthlyData.calendar_activity).length > 0 ? Math.max(...Object.values(monthlyData.calendar_activity)) : 0)}
            </span>
            <span className="text-slate-400 text-[10px] uppercase font-bold mt-1">Highest Score</span>
          </div>
        </div>
    ...
    const getFactorIcon = (factor: string) => {
      switch(factor.toLowerCase()) {
        case 'workload': case 'work': case 'studies': return Briefcase;
        case 'sleep deprivation': case 'poor sleep': case 'sleep': return Moon;
        case 'financial stress': case 'money': return AlertTriangle;
        case 'relationship issues': case 'family': return User;
        case 'health concerns': case 'health': return RunIcon;
        case 'screen time': return Smartphone;
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-[#1e2132] border border-slate-800/50 rounded-[32px] p-6 shadow-lg">
          ...
          <p className="text-slate-400 text-[13px] mb-6">Each factor's impact based on your latest check-in</p>
          
          <div className="space-y-1">
            {latestCheckinDetails ? (
              (() => {
                const sleepScore = Math.min(100, Math.max(0, Math.round((latestCheckinDetails.sleep_duration / 8) * 100)));
                const screenScore = Math.min(100, Math.max(0, Math.round(100 - (latestCheckinDetails.screen_time / 10) * 100)));
                const caffeineScore = Math.min(100, Math.max(0, Math.round(100 - (latestCheckinDetails.caffeine_intake / 5) * 100)));
                const activityScore = Math.min(100, Math.max(0, Math.round((latestCheckinDetails.physical_activity / 5) * 100)));
                
                const getMoodScore = (m: string) => {
                  const m2 = (m||'').toLowerCase();
                  if (m2 === 'happy') return 100;
                  if (m2 === 'calm') return 80;
                  if (m2 === 'neutral') return 60;
                  if (m2 === 'sad') return 40;
                  return 20;
                };
                
                const getWorkScore = (w: string) => {
                  const w2 = (w||'').toLowerCase();
                  if (w2 === 'light') return 100;
                  if (w2 === 'normal') return 80;
                  if (w2 === 'moderate') return 60;
                  if (w2 === 'heavy') return 40;
                  return 20;
                };

                const getColor = (score: number) => {
                  if (score >= 75) return "text-[#82e0aa]";
                  if (score >= 40) return "text-yellow-500";
                  return "text-rose-500";
                };
                const getBg = (score: number) => {
                  if (score >= 75) return "bg-[#82e0aa]";
                  if (score >= 40) return "bg-[#ffd700]";
                  return "bg-[#ff4b4b]";
                };

                const moodS = getMoodScore(latestCheckinDetails.mood);
                const workS = getWorkScore(latestCheckinDetails.workload);

                return (
                  <>
                    <FactorItem icon={Moon} title="Sleep Quality" impact={`${latestCheckinDetails.sleep_duration} hrs (Latest)`} percentage={sleepScore} colorClass={getColor(sleepScore)} bgClass={getBg(sleepScore)} />
                    <FactorItem icon={Smartphone} title="Screen Time Wellness" impact={`${latestCheckinDetails.screen_time} hrs (Latest)`} percentage={screenScore} colorClass={getColor(screenScore)} bgClass={getBg(screenScore)} />
                    <FactorItem icon={Coffee} title="Caffeine Management" impact={`${latestCheckinDetails.caffeine_intake} cups (Latest)`} percentage={caffeineScore} colorClass={getColor(caffeineScore)} bgClass={getBg(caffeineScore)} />
                    <FactorItem icon={RunIcon} title="Physical Activity" impact={`Level ${latestCheckinDetails.physical_activity} (Latest)`} percentage={activityScore} colorClass={getColor(activityScore)} bgClass={getBg(activityScore)} />
                    <FactorItem icon={Smile} title="Mood State" impact={latestCheckinDetails.mood || 'Neutral'} percentage={moodS} colorClass={getColor(moodS)} bgClass={getBg(moodS)} />
                    <FactorItem icon={Briefcase} title="Workload Balance" impact={latestCheckinDetails.workload || 'Light'} percentage={workS} colorClass={getColor(workS)} bgClass={getBg(workS)} />
                  </>
                );
              })()
            ) : (
              <p className="text-slate-400 text-sm">Loading your latest check-in factors...</p>
            )}
          </div>
        </div>
...
const renderHistory = () => {
    return (
      <div className="space-y-4">
        {history.length > 0 ? (
          history.map((item) => {
            const dateStr = String(item.timestamp).endsWith('Z') ? String(item.timestamp) : String(item.timestamp) + 'Z';
            const dateObj = new Date(dateStr);
            return (
            <div key={item.id} className="bg-slate-900/50 border border-slate-800 p-4 rounded-2xl flex justify-between items-center hover:bg-slate-800/50 transition-colors cursor-pointer" onClick={() => router.push(`/history/${item.id}`)}>
              <div>
                <p className="text-white font-bold">{dateObj.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</p>
                <p className="text-xs text-slate-400 mt-1">{dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
              </div>
  ...
  const renderContent = () => {
    if (activeTab === 'Home') {
        ...
        <div className="flex-1 overflow-y-auto px-6 py-12 flex flex-col min-h-full bg-[#111116]">
          ...
          <div className="flex items-center justify-between mb-10 mt-2">
            <div className="flex items-center gap-4">
              <div className="w-[60px] h-[60px] rounded-full flex items-center justify-center overflow-hidden border border-emerald-400">
            ...
            >
              ...
              {generateNotifications().length > 0 && (
                <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-[#3a355d]"></span>
              )}
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-6 relative w-full mb-12">
            ...
            >
                ...
                <span className="text-slate-300 font-medium text-[15px] mt-1">{stressCategory}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 w-full">
            <div className="bg-[#2c2c35] rounded-3xl p-5 flex flex-col items-start justify-center">
              ...
              <p className="text-slate-400 text-xs font-semibold">{t('Check-ins')}</p>
            </div>
            ...
            <div className="bg-[#2c2c35] rounded-3xl p-5 flex flex-col items-start justify-center">
              ...
              <p className="text-slate-400 text-xs font-semibold">{t('Days Streak').split(' ')[0]}</p>
            </div>
      ...
      if (showExportModal) return renderExportModal();
      return (
  ...
  const renderExportModal = () => {
    const timePeriods = [
      { id: 'Last 7 Days', size: '84 KB' },
      { id: 'Last 30 Days', size: '320 KB' },
      { id: 'Last 3 Months', size: '950 KB' },
      { id: 'All Time', size: '2.1 MB' }
    ];
    ...
    const handleGenerateReport = () => {
      ...
      const now = new Date();
      if (exportTimePeriod === 'Last 7 Days') {
        const d = new Date(now.setDate(now.getDate() - 7));
        filteredHistory = filteredHistory.filter(h => new Date(h.timestamp) >= d);
      } else if (exportTimePeriod === 'Last 30 Days') {
        const d = new Date(now.setDate(now.getDate() - 30));
        filteredHistory = filteredHistory.filter(h => new Date(h.timestamp) >= d);
      } else if (exportTimePeriod === 'Last 3 Months') {
        const d = new Date(now.setMonth(now.getMonth() - 3));
        filteredHistory = filteredHistory.filter(h => new Date(h.timestamp) >= d);
      }

      if (exportFormat === 'CSV') {
        const header = "Date,Time,Score,Stress Level\n";
        const rows = filteredHistory.map(item => {
          const d = new Date(item.timestamp);
          return `${d.toLocaleDateString()},${d.toLocaleTimeString()},${item.score},${item.stress_level}`;
        }).join("\n");
        ...
        a.href = url;
        a.download = `CortiSense_Report_${exportTimePeriod.replace(/ /g, '_')}.csv`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
      } else {
        // PDF Simulation using Print Dialog
        const printWindow = window.open('', '_blank');
        if (printWindow) {
          let html = `<html><head><title>CortiSense Report</title><style>body{font-family:sans-serif;padding:20px;color:#333;}table{width:100%;border-collapse:collapse;margin-top:20px;}th,td{border:1px solid #ddd;padding:8px;text-align:left;}th{background-color:#f2f2f2;}</style></head><body>`;
          html += `<h2>CortiSense Wellness Report - ${exportTimePeriod}</h2>`;
          html += `<p>Generated on: ${new Date().toLocaleString()}</p>`;
          html += `<table><tr><th>Date</th><th>Time</th><th>Score</th><th>Stress Level</th></tr>`;
          filteredHistory.forEach(item => {
            const d = new Date(item.timestamp);
            html += `<tr><td>${d.toLocaleDateString()}</td><td>${d.toLocaleTimeString()}</td><td>${item.score}</td><td>${item.stress_level}</td></tr>`;
          });
    ...
    return (
      <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6 bg-[#0f111a] min-h-full">
        <div className="flex items-center mb-2">
          <button onClick={() => setShowExportModal(false)} className="text-white mr-4">
            <ArrowLeft size={24} />
          </button>
        </div>
        
        <div>
          <h1 className="text-[28px] font-extrabold text-white leading-tight mb-1">Export Report</h1>
          <p className="text-slate-300 text-[15px]">Download your wellness data</p>
        </div>

        <div className="mt-8">
          <h3 className="text-white font-bold text-lg mb-4">Select Time Period</h3>
          <div className="space-y-3">
            {timePeriods.map((tp) => (
              <button
                key={tp.id}
                onClick={() => setExportTimePeriod(tp.id)}
                className={`w-full flex items-center p-4 rounded-[16px] border transition-all ${exportTimePeriod === tp.id ? 'bg-[#1e2724] border-[#82e0aa]' : 'bg-[#1c1e2b] border-[#2c2f42]'}`}
              >
                <Calendar size={20} className={exportTimePeriod === tp.id ? 'text-[#82e0aa] mr-4' : 'text-slate-400 mr-4'} />
                <span className={`font-bold flex-1 text-left ${exportTimePeriod === tp.id ? 'text-[#82e0aa]' : 'text-white'}`}>{tp.id}</span>
                <span className="text-slate-400 text-sm font-medium">{tp.size}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-8">
          <h3 className="text-white font-bold text-lg mb-4">Export Format</h3>
          <div className="grid grid-cols-2 gap-4">
            <button
              onClick={() => setExportFormat('PDF')}
              className={`flex flex-col items-center justify-center p-6 rounded-[16px] border transition-all ${exportFormat === 'PDF' ? 'bg-[#1e2724] border-[#82e0aa]' : 'bg-[#1c1e2b] border-[#2c2f42]'}`}
            >
              <div className={`mb-3 ${exportFormat === 'PDF' ? 'text-[#82e0aa]' : 'text-slate-400'}`}>
                <FileText size={28} />
              </div>
              <span className={`font-bold ${exportFormat === 'PDF' ? 'text-[#82e0aa]' : 'text-slate-300'}`}>PDF</span>
            </button>
            <button
              onClick={() => setExportFormat('CSV')}
              className={`flex flex-col items-center justify-center p-6 rounded-[16px] border transition-all ${exportFormat === 'CSV' ? 'bg-[#1e2724] border-[#82e0aa]' : 'bg-[#1c1e2b] border-[#2c2f42]'}`}
            >
              <div className={`mb-3 ${exportFormat === 'CSV' ? 'text-[#82e0aa]' : 'text-slate-400'}`}>
                <FileText size={28} />
              </div>
              <span className={`font-bold ${exportFormat === 'CSV' ? 'text-[#82e0aa]' : 'text-slate-300'}`}>CSV</span>
            </button>
          </div>
        </div>

        <div className="mt-8 bg-[#2d244a] rounded-[24px] p-6">
          <h3 className="text-white font-bold text-lg mb-5">Report Includes</h3>
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full bg-[#82e0aa] flex items-center justify-center shrink-0">
                <Check size={14} className="text-[#2d244a] font-bold" />
              </div>
              <span className="text-slate-300/80 font-medium text-sm">Daily stress scores and trends</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full bg-[#82e0aa] flex items-center justify-center shrink-0">
                <Check size={14} className="text-[#2d244a] font-bold" />
              </div>
              <span className="text-slate-300/80 font-medium text-sm">Physiological data (heart rate, sleep)</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full bg-[#82e0aa] flex items-center justify-center shrink-0">
                <Check size={14} className="text-[#2d244a] font-bold" />
              </div>
              <span className="text-slate-300/80 font-medium text-sm">Psychological assessments</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full bg-[#82e0aa] flex items-center justify-center shrink-0">
                <Check size={14} className="text-[#2d244a] font-bold" />
              </div>
              <span className="text-slate-300/80 font-medium text-sm">Lifestyle factors and patterns</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-full bg-[#82e0aa] flex items-center justify-center shrink-0">
                <Check size={14} className="text-[#2d244a] font-bold" />
              </div>
              <span className="text-slate-300/80 font-medium text-sm">AI insights and recommendations</span>
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          className="w-full mt-6 bg-[#82e0aa] hover:bg-[#6eb171] text-[#111116] font-bold text-lg py-4 rounded-[16px] shadow-lg transition-colors active:scale-95"
        >
          Generate Report
        </button>
  ...
  const renderNotificationsPanel = () => {
    ...
    return (
      <div className="fixed inset-0 z-[100] flex justify-end">
        ...
        <div className="relative w-full max-w-sm h-full bg-[#111116] border-l border-slate-800 flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
          <div className="p-6 border-b border-slate-800 flex items-center justify-between">
            <h2 className="text-xl font-bold text-white flex items-center gap-2">
              <Bell size={24} className="text-emerald-400" />
              Notifications
            </h2>
            <button onClick={() => setShowNotifications(false)} className="text-slate-400 hover:text-white p-2">
              ✕
            </button>
          </div>
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
            {notifications.map((notif) => (
              <div key={notif.id} className="bg-[#1c1e2b] rounded-2xl p-4 border border-slate-800/50 hover:border-emerald-500/30 transition-colors">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="text-white font-bold text-sm pr-4">{notif.title}</h3>
                  <span className="text-slate-500 text-[10px] font-medium whitespace-nowrap">{formatTimeAgo(notif.time)}</span>
                </div>
