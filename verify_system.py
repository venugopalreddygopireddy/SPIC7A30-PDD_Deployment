...
DB_URL = "postgresql://postgres:venugopal5242@localhost:5432/cortisense_db"
...
def test_system():
    ...
    if actions:
        ...
        print("Action card sync verified.")
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT reset_otp FROM users WHERE email = %s", (email,))
        otp = cur.fetchone()[0]
        cur.close()
        conn.close()
        ...
        resp = requests.post(f"{BASE_URL}/verify-otp", json={"email": email, "otp": otp})
        assert resp.status_code == 200, resp.text
        
        resp = requests.post(f"{BASE_URL}/reset-password", json={"email": email, "otp": otp, "new_password": "newpassword123"})
        assert resp.status_code == 200, resp.text
        
        resp = requests.post(f"{BASE_URL}/login", json={"email": email, "password": "newpassword123"})
        assert resp.status_code == 200, resp.text
        print("OTP workflow verified.")
